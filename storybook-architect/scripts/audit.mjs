#!/usr/bin/env node
// Optional experimental source survey. Prefer a project's maintained audit tooling.
// Usage:
...
<Meta title="Audit/${title}" tags={['sb-architect-audit', '!manifest', '!autodocs', '!test']} />
...
writeFileSync(join(OUT, 'Overview.mdx'), page('Overview', `# Source survey — ${new Date(findings.generatedAt).toDateString()}

Use these lists to choose a few candidates to inspect. Confirm the consumer impact before
changing a component or adding a token. An empty list means this scanner found no matches,
not that the system has no problems.

${table(['Metric', 'Value'], [
  ['Literal value matches', `${metrics.literalValueMatches} in ${metrics.filesWithLiteralValues} files`],
  ['Component files with a matching story', metrics.storiedComponentRatio === null ? 'unknown - no component exports recognized' : `${metrics.storiedComponentRatio}% of ${metrics.componentsScanned} recognized files`],
...
source text; a scanned stylesheet counts separately from components importing it.
...
This report is generated; ask your agent to rerun the same scoped survey to refresh it.`));
...
writeFileSync(join(OUT, 'Duplicates.mdx'), page('Duplicates', `# Components worth comparing\n\nSimilar names or overlapping prop names can suggest a comparison. Inspect behavior, types, ownership, and consumers before deciding whether sharing an implementation would help.\n\n${table(['A', 'B', 'Why flagged', 'Prop-name overlap', 'Shared names'], duplicates.map((d) => [d.a, d.b, d.reason, d.propOverlap, d.sharedProps.slice(0, 6).join(', ')]))}`));

writeFileSync(join(OUT, 'Coverage.mdx'), page('Coverage', `# Files without a matching story\n\nCheck whether consumers need an example and whether one exists under another name. This scanner recognizes at most one component per file. The ordering uses import matches inside the scanned root; it misses external consumers and does not establish reuse or documentation need.\n\n${table(['Recognized component', 'Import matches in scope', 'File'], uncovered.slice(0, 40).map((u) => [u.component, u.usedIn, u.file]))}`));
...
writeFileSync(join(OUT, 'NamingDrift.mdx'), page('Naming drift', `# Vocabulary to review\n\nThese words occur in the scanned source. Establish their roles and scope before treating them as competing names: primary, brand, and accent can describe different concepts.\n\n${table(['Names found'], drift.map((d) => [d.competing.join(' / ')]))}`));

console.log(JSON.stringify(metrics, null, 2));
console.log(`\nWrote ${OUT}/findings.json and 6 optional Storybook report pages. These are review candidates, not a quality verdict. See the skill's scanner guide before integrating the pages.`);
